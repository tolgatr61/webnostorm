...
<head>
    ...
    <title><?php echo $site['name']; ?> - Confirmer Achat</title>
...
<!-- END : Navbar -->
    ...
    <div class="nk-store nk-store-checkout">
        ...
        <h3 class="nk-decorated-h-2"><span><span class="text-main-1">Votre</span> commande</span></h3>
        <div class="nk-gap"></div>
        <div class="table-responsive">
            <table class="nk-table nk-table-sm">
                <thead class="thead-default">
                    <tr>
                        <th class="nk-product-cart-title">Produit</th>
                        <th class="nk-product-cart-title">Amount</th>
                        <th class="nk-product-cart-total">Total</th>
                        <th class="nk-product-cart-character">Personnage</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    $req->execute([$_SESSION["AccountId"]]);
                    $req2 = $conn->prepare("SELECT * FROM Character WHERE CharacterId = ?");
                    $req1 = $conn->prepare("SELECT * FROM Item WHERE VNum = ?");
                    $req3 = $conn->prepare("SELECT * FROM ShoppingCartDreamTrix WHERE Amount = ?");
                    $_SESSION["TotalAmount"] = 0;
                    while($result = $req->fetch()):
                        $req1->execute([$result["VNum"]]);
                        $result1 = $req1->fetch();
                        $req2->execute([$result["CharacterId"]]);
                        $result2 = $req2->fetch();
                        $req3->execute([$result["Amount"]]);
                        $result3 = $req3->fetch();
                        ?>
                        <tr>
                            ...
                            <td class="nk-product-cart-character">
                                <?= $result2["Name"]; ?>
                            </td>
                        </tr>
                        <?php
                        $_SESSION["TotalAmount"] += $result3["Amount"];
                    endwhile;
                    ?>
        ...
        <a class="nk-btn nk-btn-rounded nk-btn-color-main-1" href="buy.php">Confirmer la commande</a>
    </div>
</div>
<div class="nk-gap-2"></div>
