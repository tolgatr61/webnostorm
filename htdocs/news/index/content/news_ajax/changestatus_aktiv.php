...
if(isset($_POST['commentaryID'])){
	...
	$stmt = $conn->prepare("SELECT TOP 1 COUNT(*) AS AnzahlX FROM $db_Account WHERE $db_Account.[Name] = :xname AND $db_Account.[Password] = :xpass AND $db_Account.[Authority] >= 2");
	$stmt->bindParam(':xname', $commentary_username);
	$stmt->bindParam(':xpass', $commentary_passwort);
	if($stmt->execute()){
		$row = $stmt->fetch();
		if ($row['AnzahlX'] == 1){
			$stmt = $conn->prepare("UPDATE $db_Comm SET $db_Comm.CommEnable = 1 WHERE CommenId = :commentary_ID");
